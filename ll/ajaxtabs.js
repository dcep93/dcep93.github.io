...
ddajaxtabssettings.loadstatustext =
  "<img src='/images/misc/loading.gif' /> Requesting content...";
...
function ddajaxtabs(tabinterfaceid, contentdivid) {
  ...
  this.defaultHTML = "";
  this.defaultIframe =
    '<iframe src="about:blank" marginwidth="0" marginheight="0" frameborder="0" vspace="0" hspace="0" class="tabcontentiframe" style="width:100%; height:3000px; min-height: 100px"></iframe>';
  this.defaultIframe = this.defaultIframe.replace(
    /<iframe/i,
    '<iframe name="' + "_ddajaxtabsiframe-" + contentdivid + '" '
  );
  this.revcontentids = [];
...
ddajaxtabs.connect = function (pageurl, tabinstance) {
  ...
  var ajaxfriendlyurl = pageurl.replace(
    /^https:\/\/[^\/]+\//i,
    "https://" + window.location.hostname + "/"
  );

  // dcep93 fix
  ajaxfriendlyurl = `https://dcep93--first2know-app.modal.run/proxy/${document.baseURI}${pageurl}`;
  //
...
ddajaxtabs.prototype = {
  expandit: function (tabid_or_position) {
    ...
    try {
      if (
        typeof tabid_or_position == "string" &&
        document.getElementById(tabid_or_position).getAttribute("rel")
      )
        tabref = document.getElementById(tabid_or_position);
      else if (
        parseInt(tabid_or_position) != NaN &&
        this.tabs[tabid_or_position].getAttribute("rel")
      )
  ...
  cycleit: function (dir, autorun) {
    if (dir == "next") {
      var currentTabIndex =
        this.currentTabIndex < this.hottabspositions.length - 1
  ...
  expandtab: function (tabref) {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      this.getselectedClassTarget(this.tabs[i]).className =
        this.tabs[i].getAttribute("href") == tabref.getAttribute("href")
          ? "selected"
          : "";
    }
  ...
  expandrevcontent: function (associatedrevids) {
    ...
    for (var i = 0; i < allrevids.length; i++) {
      document.getElementById(allrevids[i]).style.display =
        associatedrevids.indexOf("," + allrevids[i] + ",") != -1
          ? "block"
          : "none";
    }
  },
  ...
  init: function (automodeperiod) {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      ...
      if (this.tabs[i].getAttribute("rel")) {
        ...
        if (this.tabs[i].getAttribute("rev")) {
          this.revcontentids = this.revcontentids.concat(
            this.tabs[i].getAttribute("rev").split(/\s*,\s*/)
          );
        }
        if (
          selectedtabfromurl == i ||
    ...
    if (
      parseInt(this.automodeperiod) > 500 &&
      this.hottabspositions.length > 1
    ) {
